...
Date: 9 July 2026
Last Updated: 12 July 2026 (Phase 6 Memory Intelligence COMPLETE — Foundation, Person-Scoped, Short-Term, Long-Term, Personal Context, Behaviour Learning. All committed and pushed: v0.15/v0.15.1 `939ca97`, v0.15.2–v0.15.4 `986c4e7`, v0.15.5 `ae544ba`. 39 files / 191 tests. Next: v0.16 Face Recognition. See SESSION_HANDOFF_2026-07-12.md)
...
| Item | Status |
|---|---|
| Last fully complete milestone | v0.15.5 — Behaviour Learning (confirm-gated inferred facts from conversation + behaviour; never safety; `source: 'inferred'`, decay-eligible). Caps off Phase 6 Memory Intelligence, which also includes v0.15.4 Personal Context (on-prem, memory-aware AI), v0.15.3 Long-Term, v0.15.2 Short-Term, v0.15.1 Person-Scoped, and v0.15 Foundation. |
...
| Test baseline | 39 test files, 191 tests — Build PASS, Release Check PASS, UI PASS (memory round-trip, person-scoping, categorisation/safety, working-memory seed, personal-context on-prem-only, and the inferred-candidate confirm flow all verified live). All committed and pushed through `ae544ba`. |
| Next milestone | v0.16 — Face Recognition (wires `MemoryIntelligenceService.setActivePerson` so personal context, working-memory seeding, and behaviour learning follow whoever the camera identifies) |
...
- `Engineering_Manifest_v0.15.4_Personal_Context.md`
- `Engineering_Manifest_v0.15.5_Behaviour_Learning.md`
...
See `MVCH_Master_Version_Control_History.md` (same folder) for the full versioned roadmap through v0.21.4 (MARS Mk1 Production Release), now including Phase 9 Multi-Camera / Distributed Monitoring (v0.18.x) ahead of Robot Control. v0.14.4 AI Reasoning Layer is complete. **Phase 6 Memory Intelligence is COMPLETE**: Foundation (v0.15), Person-Scoped Memory (v0.15.1), Short-Term Memory Engine (v0.15.2), Long-Term Memory Engine (v0.15.3), Personal Context (v0.15.4, on-prem memory-aware AI), Behaviour Learning (v0.15.5, confirm-gated inferred facts). Immediate next step: **v0.16 Face Recognition**, which wires `MemoryIntelligenceService.setActivePerson` so personal context, working-memory seeding, and behaviour learning all follow whoever the camera identifies.
...
- **v0.15.2–v0.15.5 — DONE and pushed (12 July 2026).** v0.15.2 `WorkingMemoryService` (seed + promote via the first real `CapabilityRouter` memory-write dispatch). v0.15.3 `MemoryClassifier` + `LongTermMemoryEngine` (categorisation incl. protected `safety`, salience, safety-aware retention). v0.15.4 `PersonalContextService` (on-prem memory-aware AI; cloud gets no personal context). v0.15.5 `InferenceParser` + `BehaviourLearningService` (confirm-gated inferred facts; never safety). Commits `986c4e7` (v0.15.2–v0.15.4) and `ae544ba` (v0.15.5). 39 files / 191 tests.
- `CapabilityRouter.js` performs a real memory **write** when `plan.memoryOp === 'write'` (used by WorkingMemory promotion and BehaviourLearning confirm). It is still not wired into the live turn loop otherwise.
- **v0.16 Face Recognition** — the immediate next milestone: will call `MemoryIntelligenceService.setActivePerson(personId)` once an identity is confirmed, so untagged "my …", personal context, working-memory seeding and behaviour learning all follow the recognised person. Hook already exists.
