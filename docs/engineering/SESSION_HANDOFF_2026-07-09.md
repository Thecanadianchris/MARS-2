...
**Still open, needs Christian or a working sandbox:**
- Continue v0.14.2 to completion / scope v0.14.3 — real feature work, not yet started this session.
- Any new code written from here (including this Notes rename) needs `npm test` / `npm run build` / `npm run release-check` run by Christian before commit, per the project's own workflow gate, since I still can't run those myself.
