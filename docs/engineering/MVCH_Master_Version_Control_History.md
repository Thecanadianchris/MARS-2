...
| Version | Status | Date | Description |
|---|---|---|---|
| **v0.16** | 🚧 In Progress | 14 July 2026 | Face Recognition Foundation. `RecognitionCandidate`/`SEARCHING` state/`setActivePerson()` payoff all wired and live-verified with Christian's own face (see below). **Matcher upgraded same day**: the original landmark-geometry matcher (`FaceSignatureEngine`, 8 proportion ratios) was replaced with a real on-device face-embedding model (`@vladmandic/face-api`, 128-d descriptors — `FaceEmbeddingService`/`FaceEmbeddingEngine`) after a live test showed the geometry approach misidentified an unenrolled person (Ann) as the only enrolled profile (Christian) — a real safety-relevant finding, not a wiring bug. `npm run release-check` PASS (13 July) on the pre-upgrade code; **not yet re-run since the matcher swap** — needs `npm install` (new dependency) then `npm run release-check` again. Back to 🚧 rather than ✅ until that passes *and* the actual motivating re-test (Ann enrolled for real, confirmed distinguishable from Christian live) has happened — see `ENGINEERING_BACKLOG.md`'s "Known safety-relevant limitation" entry for the full account. |
